...
const PitchCanvas = dynamic(() => import("@/components/three/PitchCanvas"), {
  ssr: false,
});
...
export default function SquadBuilder() {
  ...
  // fantasyLeague: must joinLeague() once, then submitLineup(tokenIds, captain).
  async function submit() {
    ...
    if (deployed.fantasyLeague && isConnected) {
      try {
        if (!hasJoined) {
          ...
          flash("Joined league ✓ — submit again to set your XI");
        } else {
          const tokenIds = (
            Object.values(lineupPlayers).filter(Boolean) as Player[]
          ).map((p) => p.tokenId);
          await tx.writeContractAsync(
            txSubmitLineup(tokenIds, captain.tokenId)
          );
          flash("Lineup submitted on-chain ✓");
        }
      } catch {
    ...
    flash("Lineup submitted on-chain ✓ (demo)");
  }
    ...
    <div className="py-6">
        ...
        <div className="glass p-4 sm:p-6">
          <div className="mb-3 flex items-center justify-between">
            <span className="label-mono">
              {formation} · {filled}/{slots.length} picked
            </span>
          ...
          {view === "3d" ? (
            <PitchCanvas
              className="h-[460px] w-full overflow-hidden rounded-2xl bg-pitch-950"
              slots={slots}
      ...
      <TxToast
        ...
        labels={{
          ...
          success: "Lineup live on X Layer",
        }}
      />
    </div>
  );
