...
2. **(If league #3 is no longer open)** open a fresh one and re-point the app:
   ```bash
   pnpm --filter @aff/contracts open-league:testnet   # prints the new id, writes NEXT_PUBLIC_LEAGUE_ID
   pnpm --filter @aff/web build && pnpm web            # rebuild so the new id is inlined
   ```
...
4. **Settle so claim works on camera:**
   ```bash
   LEAGUE_ID=3 pnpm --filter @aff/contracts settle:testnet   # reports points → lock → settle
   ```
...
5. **(Optional) refresh the README proof block** with a brand-new self-contained lifecycle:
   ```bash
   pnpm --filter @aff/contracts seed:testnet   # creates its own league, runs create→…→claim
   ```
